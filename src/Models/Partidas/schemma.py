...
class partidas():
    ...
    def __Soup__(self, time : str) -> list:
        ...
        list_yellowcards = soup.select(" .trajectory .table_parents tbody tr.parent_row td:nth-of-type(2) b")

        for i in range(len(list_temporadas)):
            list_.append(
                {
                    f"Temporada {list_temporadas[i].get_text()}" : f"Partidas Jogadas - {list_yellowcards[i].get_text()}"
                }
            )
